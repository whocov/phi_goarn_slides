...
#   
#   h2 <- glue::glue("From of {format(Sys.Date() - 7, '%d %b %Y')} to {format(Sys.Date(), '%d %b %Y')}")
#   
...
#     # Published
#     pub <- prod.tracker %>% 
#       select(disease_condition, country, rra_pub_date, eis_pub_date, don_pub_date) %>%
...
#       mutate(rra_pub_date2 = ifelse(rra_pub_date %within% date.range, 'X', ''),
...
#       select(-rra_pub_date, -eis_pub_date, -don_pub_date)
#     
#     
#     #In progress
...
#       filter(is.na(rra_pub_date) | is.na(eis_pub_date) | is.na(don_pub_date)) %>%
#       mutate(rra_pub_date2 = ifelse(need_rra & is.na(rra_start_date) & is.na(rra_pub_date), 'X*', ''),
...
#       select(-need_rra, -need_eis, -need_don, 
...
#       as.data.frame()
#     
#     colnames(prod.table) <- c('disease_condition', 'country', 'rra', 'eis', 'don')
#     
...
#       as.data.frame()
#     
#     colnames(prod.table) <- c('disease_condition', 'country', 'rra', 'eis', 'don', 'other')
#     
#     prod.table <- prod.table %>%
#       # filter to show just DON related info-products
#       filter(!is.na(don)) %>% 
#       select(-rra, -eis) %>%
#       flextable() %>%
#       style(pr_t=fp_text(font.size=14), part='all') %>%
...
#### replaced from the AEM slides code
slide_products <- function(pres, data, data2) {
  ...
  if (is.null(prod2)){
    ...
    prod.tracker <- prod.tracker %>%
      mutate(across(c(rra_pub_date, eis_pub_date, don_pub_date, p_rra_pub_date),
                    ~ suppressWarnings(lubridate::as_date(.x))))
    pub <- prod.tracker %>% 
      select(disease_condition, country, countries_if_multiple_or_region, rra_pub_date, eis_pub_date, don_pub_date, p_rra_pub_date) %>%
      # filter(!is.na(rra_pub_date) | !is.na(eis_pub_date) | !is.na(don_pub_date)) %>%
      filter(rra_pub_date %within% date.range | eis_pub_date %within% date.range | 
               don_pub_date %within% date.range | p_rra_pub_date %within% date.range) %>%
      mutate(country = ifelse(country == 'Multiple', countries_if_multiple_or_region, country),
             rra_pub_date2 = ifelse(rra_pub_date %within% date.range, 'X', ''),
             eis_pub_date2 = ifelse(eis_pub_date %within% date.range, 'X', ''),
             don_pub_date2 = ifelse(don_pub_date %within% date.range, 'X', ''),
             p_rra_pub_date2 = ifelse(p_rra_pub_date %within% date.range, 'X', '')) %>%
      mutate(across(rra_pub_date:p_rra_pub_date, ~ ifelse(is.na(.x), '',.x))) %>%
      select(-countries_if_multiple_or_region, -rra_pub_date, -eis_pub_date, -don_pub_date, -p_rra_pub_date)
    ...
    inprogress <- prod.tracker %>%
      ...
      select(disease_condition, country, countries_if_multiple_or_region,
             need_rra, rra_pub_date, need_eis, eis_pub_date, need_don, don_pub_date,
             rra_start_date, eis_start_hq_receive_date, don_start_date,need_public_rra,  p_rra_pub_date, p_rra_start_date) %>%
      filter(is.na(rra_pub_date) | is.na(eis_pub_date) | is.na(don_pub_date)| is.na(p_rra_pub_date)) %>%
      mutate(country = ifelse(country == 'Multiple', countries_if_multiple_or_region, country),
             rra_pub_date2 = ifelse(need_rra & is.na(rra_start_date) & is.na(rra_pub_date), 'X*', ''),
             eis_pub_date2 = ifelse(need_eis & is.na(eis_start_hq_receive_date) & is.na(eis_pub_date), 'X*', ''),
             don_pub_date2 = ifelse(need_don & is.na(don_start_date) & is.na(don_pub_date), 'X*', ''),
             p_rra_pub_date2 = ifelse(need_public_rra & is.na(p_rra_start_date) & is.na(p_rra_pub_date), 'X*', '')) %>%
      mutate(rra_pub_date2 = ifelse(need_rra & is.na(rra_pub_date) & !is.na(rra_start_date), 'X', rra_pub_date2),
             eis_pub_date2 = ifelse(need_eis & is.na(eis_pub_date) & !is.na(eis_start_hq_receive_date), 'X', eis_pub_date2),
             don_pub_date2 = ifelse(need_don & is.na(don_pub_date) & !is.na(don_start_date), 'X', don_pub_date2),
             p_rra_pub_date2 = ifelse(need_public_rra & is.na(p_rra_pub_date) & !is.na(p_rra_start_date), 'X', p_rra_pub_date2)) %>%
      select(-countries_if_multiple_or_region,
             -need_rra, -need_eis, -need_don, - need_public_rra, 
             -rra_pub_date, -eis_pub_date, -don_pub_date, -p_rra_pub_date, -rra_start_date, 
             -eis_start_hq_receive_date, -don_start_date, -p_rra_start_date)
    ...
    prod.table <- rbind(c('Published','',length(which(pub$rra_pub_date2=='X')),
                          length(which(pub$eis_pub_date2=='X')),
                          length(which(pub$don_pub_date2=='X')),
                          length(which(pub$p_rra_pub_date2=='X'))), 
                        pub, 
                        c('In progress','',length(which(inprogress$rra_pub_date2=='X' | inprogress$rra_pub_date2=='X*')),
                          length(which(inprogress$eis_pub_date2=='X' | inprogress$eis_pub_date2=='X*')),
                          length(which(inprogress$don_pub_date2=='X' | inprogress$don_pub_date2=='X*')),
                          length(which(inprogress$p_rra_pub_date2=='X' | inprogress$p_rra_pub_date2=='X*'))), 
                        inprogress) %>% 
      as.data.frame()
